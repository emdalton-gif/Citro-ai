// api/sub-add-property.js
// Adds a monitored property to an Enterprise subscriber's account.
// Enforces the 10-property limit.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (properties.length >= 10) {
      return res.status(400).json({ error: 'Maximum of 10 properties reached. Remove a property to add a new one.' });
    }

    const {
      name, website, industry, specific_product, offer, buyer, geo, competitors, personas
    } = req.body;
    ...
    const property = {
      ...
      specific_product: (specific_product || '').trim(),
      offer:            (offer || '').trim(),
      buyer:            (buyer || '').trim(),
      geo:              (geo || '').trim(),
    ...
    console.error('sub-add-property error:', err.message);
