// api/sub-get-property-runs.js
// Returns run history for a specific Enterprise property.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const runs = await upstashGet(`subscriber-runs:${email}:${propertyId}`) || [];
    res.json({ runs: runs.slice(0, 50) });
  } catch (err) {
    console.error('sub-get-property-runs error:', err.message);
    res.status(500).json({ error: 'Could not load property runs' });
  }
};
